...
export function SurveyPage() {
  ...
  const [currentQ, setCurrentQ] = useState(isResume && answeredCount < personalityQuestions.length ? answeredCount : 0);

  const [answers, setAnswers] = useState<Record<string, number>>(isResume ? savedAnswers : {});
    ...
    if (INSIGHT_AFTER.includes(currentQ) && !insightRoundsShown.has(currentQ)) {
      ...
      try {
        ...
        const matchedJobs = aiTop3Majors.map((aiJob: any) => {
          ...
          return foundJob || { 
            id: aiJob.major.replace(/\s+/g, '-').toLowerCase(), // Fallback ID
            name: aiJob.major, 
  ...
  const handleEarlySubmit = async (selectedJob: any) => {
    ...
    // Đánh dấu ngành mà user vừa click chọn vào localStorage
    localStorage.setItem("selectedCareerId", selectedJob.id);
    localStorage.setItem("surveyIsInterim", "true");
    const payload = calcModelPayload(answers);
    try {
      // Vẫn gửi về Backend để lấy danh sách đầy đủ lưu vào Dashboard
      ...
      // Chuyển hướng thẳng qua Dashboard
      navigate("/dashboard");
    } catch {
      alert("Không thể kết nối Backend AI.");
    } finally {
        ...
        <div className="max-w-2xl mx-auto">
          ...
          <div className="mb-6">
            <div className="flex justify-between text-xs text-gray-400 mb-1">
              <span>Câu hỏi</span><span>Phân tích sơ bộ</span><span>Tính cách</span><span>Đánh giá</span><span>Sở thích / Kỹ năng</span>
            </div>
            ...
            <div className="bg-white rounded-2xl shadow-lg p-8">
              ...
              {insightLoading ? (
                ...
                <>
                  ...
                  {/* 🌟 HIỂN THỊ TOP 3 JOB PREVIEW ĐÃ CHỈNH SỬA LẠI ĐỂ CÓ THỂ CLICK ĐƯỢC 🌟 */}
                      ...
                      {prelimJobs.map((job, index) => (
                        ...
                        >
                          <div className="flex items-center gap-2 mb-2">
                            ...
                            <h4 className="font-bold text-blue-700 group-hover:text-purple-700 text-sm leading-tight transition-colors">{job.name}</h4>
                          </div>
